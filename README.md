...
No test invokes shutdown, runAs, or arbitrary applications. The native fixture restores clipboard contents and closes only its owned window. Windows desktop behavior is verified in a disposable runner; compatibility with every third-party desktop application still depends on that application's controls and privileges.
